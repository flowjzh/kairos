...
struct SessionRegistryTests {
    ...
    func resolveAfterRegister() async {
        ...
        let flush = await r.register("k1", source: "claude-code", externalId: "s1")
        #expect(flush == nil)
        #expect(await r.resolve("k1") == SessionRegistry.Target(source: "claude-code", externalId: "s1"))
    }
    ...
    func bufferedFocusReturnedOnceOnRegister() async {
        ...
        #expect(await r.register("k1", source: "claude-code", externalId: "s1") == nil)   // consumed
    }
    ...
    func bufferLatestWins() async {
        ...
        let flush = await r.register("k1", source: "s", externalId: "e")
    ...
    func lastRegisterWins() async {
        let r = SessionRegistry()
        _ = await r.register("k1", source: "claude-code", externalId: "s1")
        _ = await r.register("k1", source: "claude-code", externalId: "s2")
        #expect(await r.resolve("k1")?.externalId == "s2")
    }
}
...
struct FocusReportTests {
    ...
    private func open(_ store: Store, externalId: String = "s1", kid: String? = nil) async throws {
        _ = await d.handle(env(.activitiesOpen, try Wire.encodeValue(
            ActivitiesOpenParams(source: "claude-code", externalId: externalId, project: nil,
    ...
    private func focusEvents(_ store: Store) async throws -> [Event] {
        try await store.loadEvents().filter { $0.kind == .aiFocus || $0.kind == .aiBlur }
    }

    @Test
    func mappedBlurAppendsAiBlurOnActivity() async throws {
        let store = try Store(path: ":memory:")
        try await open(store, externalId: "s1", kid: "k1")
        _ = try await report(store, kid: "k1", focused: false, ts: now() + 5)
        let ev = try await focusEvents(store)
        #expect(ev.map(\.kind) == [.aiBlur])
        #expect(ev[0].ts == now() + 5)
        #expect(ev[0].activityId == (try await store.findActivity(source: "claude-code", externalId: "s1")))
    }

    @Test
    func mappedFocusAppendsAiFocus() async throws {
        let store = try Store(path: ":memory:")
        try await open(store, kid: "k1")
        _ = try await report(store, kid: "k1", focused: true, ts: now() + 1)
        #expect(try await focusEvents(store).map(\.kind) == [.aiFocus])
    }
    ...
    func focusBeforeMappingBuffersThenFlushesOnHook() async throws {
        ...
        try await open(store, externalId: "s1", kid: "k1")     // SessionStart registers -> flush
        let ev = try await focusEvents(store)
        #expect(ev.map(\.kind) == [.aiBlur])
    ...
    func anyHookRegistersMappingNotJustSessionStart() async throws {
        let store = try Store(path: ":memory:")
        try await open(store, externalId: "s1", kid: nil)      // opened, but mapping unknown
        ...
        #expect(try await focusEvents(store).map(\.kind) == [.aiBlur])
    }
