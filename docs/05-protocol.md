...
- **Socket:** `~/.kairos/daemon.sock` (Unix domain; no TCP port). The runtime dir is overridable via `$KAIROS_RUNTIME_DIR` (socket + spool) so a dev and a release daemon can run side by side; the store lives separately under `~/Library/Application Support/Kairos`, overridable via `$KAIROS_DATA_DIR`. Both default when unset (release); a separate instance (e.g. dev) points them at isolated dirs via its launch config — the daemon reads only the env, with no dev/release branch.
