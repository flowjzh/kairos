...
- **Socket:** `~/.kairos/daemon.sock` (Unix domain; no TCP port). The runtime dir is overridable via `$KAIROS_RUNTIME_DIR` (socket + spool) so a dev and a release daemon can run side by side; the store lives separately under `~/Library/Application Support/Kairos`, its path baked into the app bundle (the daemon reads a `KairosDataDir` Info.plist key; the dev app sets it at build). Only the runtime axis is env-driven — a separate instance is just different dirs supplied by the build, with no dev/release branch in the code.
