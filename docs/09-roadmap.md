...
35. **Dev and release are separate instances: runtime/data split behind two env-dir overrides; the running code has no dev/release branch.** State is resolved through one indirection point (`KairosPaths` in Swift, `runtime_dir()` in Rust `libs/client`) along two axes: **runtime** (socket + spool) from `$KAIROS_RUNTIME_DIR` else `~/.kairos`, and **data** (the SQLite store) from `$KAIROS_DATA_DIR` else `~/Library/Application Support/Kairos`. The split is sandbox-forward: a future App-Store sandbox points runtime at a Group Container (the socket must stay reachable by the non-sandboxed Rust CLI/plugin) and data at the App Container — so the two are kept distinct now, not merged into one root. `_DIR` (not `_PATH`) follows XDG (`XDG_RUNTIME_DIR` is where sockets live). A separate instance is *only* a different pair of dirs: the daemon never asks "am I dev?", it just reads the env (or the release defaults). Dev/release divergence lives entirely in the **build**, not the code — `make install-dev` bakes the `-dev` dirs into the dev app's `LSEnvironment` (double-click / `make dev`) and its LaunchAgent's `EnvironmentVariables` (`make start-dev`); the two apps share one identical binary. This repo routes its own plugin + `kairos claude` to the dev daemon via `.claude/settings.local.json` `env` (Claude injects it into hook subprocesses) plus a matching shell export. Both LaunchAgents are opt-in (`RunAtLoad=false KeepAlive=false`) — the daemon runs only when started (`make start` / `make start-dev`), never at login; `make clean-dev` wipes only the dev instance, so developing never touches real freelance data. Release binaries are stripped (Swift via `strip -x`, Rust via `[profile.release] strip`); dev keeps symbols.
