...
35. **Dev and release are separate instances: runtime env-dir override + bundle-baked data dir; the running code has no dev/release branch.** State is resolved through one indirection point (`KairosPaths` in Swift, `runtime_dir()` in Rust `libs/client`) along two axes: **runtime** (socket + spool) from `$KAIROS_RUNTIME_DIR` else `~/.kairos` — env-driven, because the non-bundle Rust CLI/hook must target the same socket; and **data** (the SQLite store) from the bundle's `KairosDataDir` Info.plist key else `~/Library/Application Support/Kairos` — bundle-baked, because only the daemon reads it (and the daemon *is* the bundle), so the value rides in the app's own Info.plist rather than the environment: one fewer var to sync, and the data dir is immune to shell-env leakage. The split is sandbox-forward: a future App-Store sandbox points runtime at a Group Container (the socket must stay reachable by the non-sandboxed Rust CLI/plugin) and data at the App Container — so the two are kept distinct now, not merged into one root. `_DIR` (not `_PATH`) follows XDG (`XDG_RUNTIME_DIR` is where sockets live). A separate instance is *only* a different pair of dirs: the daemon never asks "am I dev?", it just reads its env (runtime) and its bundle key (data), or the release defaults. Dev/release divergence lives entirely in the **build**, not the code — `make install-dev` bakes the `-dev` dirs into the dev app's bundle (`KAIROS_RUNTIME_DIR` via `LSEnvironment`, the data dir as a `KairosDataDir` Info.plist key), so any launch path (double-click, `make dev`, `make start-dev`) reaches the dev instance; the two apps share one identical binary. This repo routes its own plugin + `kairos claude` to the dev daemon via `.claude/settings.local.json` `env` (`KAIROS_RUNTIME_DIR` only — the hook never reads the data dir; Claude injects it into hook subprocesses) plus a matching shell export. Both LaunchAgents are opt-in (`RunAtLoad=false KeepAlive=false`) — the daemon runs only when started (`make start` / `make start-dev`), never at login; `make clean-dev` wipes only the dev instance, so developing never touches real freelance data. Release binaries are stripped (Swift via `strip -x`, Rust via `[profile.release] strip`); dev keeps symbols.
36. **A precise-timer scheduler reconciles manual-activity `state` to its focus/blur timeline; no schedule table.** A scheduled meeting is its future-dated `focus@start` / `blur@end` events in the append-only log — that *is* the durable plan, so no separate schedule table is needed. `ScheduleController` (mirroring `IdleSamplerController`) arms one one-shot `DispatchSourceTimer` for the next future crossing, reconciles `state` on fire (future → `stopped`/dormant, ongoing → `active`, ended → `stopped`), and re-arms; a `store.changes()` subscription re-arms when a new schedule is written. `state` becomes the **single authority** for menu placement and auto-catch candidacy, fixing the desync where an ongoing timed meeting (stored `stopped` at creation) was excluded from the auto-catch backdrop list while the menu showed it as Ongoing (the prior commit's time-override in `ActivityBuckets` papered over display only). `ActivityBuckets` and the auto-catch query simplify (they trust `state`); the takeover *notification* stays in `DaemonModel.refresh` (it fires via the refresh the state-flip's `signalChange` induces — no notify code in the controller). Restart-safe: `start()` re-reconciles from the log, so crossings missed while down are recovered (only a real-time notification during downtime is lost — unavoidable). Chosen over polling (5–30 s latency wasted on rare events) and over a schedule table (the log already holds the plan); `Store.changes()` became multi-consumer broadcast so the controller subscribes independently of the menu. The menu's 30 s safety tick remains as a backstop, but the scheduler owns the crossings. *(Superseded by ADR 37: the scheduler was itself a cache of the log — placement is now derived live, and `state` collapsed to a visibility flag.)*
37. **Placement is derived live from the event log; `state` is just a visibility flag; live reads are windowed.** ADR 36's `ScheduleController` kept `activities.state` warm against the focus/blur timeline — a second derivation alongside the live `GlobalState.reduce`, i.e. a cache of the log policed by a timer. That is removed: placement (Ongoing / Upcoming / Recent) is a **pure fold over events**, exactly as the focus pointer already is. `state` collapses from active/stopped/archived to **visible(0) / archived(1)** — once timing is derived, active-vs-stopped carries no information (a ✕'d manual and a derived-ended manual behave identically; an exited auto is simply archived on `activitiesStop`). One query (`state != archived`) feeds all three sections; `ActivityBuckets.partition(visible:events:now:)` splits them. "Ended" is a **manual-only** rule (a manual's blur *is* its end — scheduled `blur@end` or the ✕-now blur; an auto's blurs are transient, so it stays Ongoing while visible); the `sources.manual` flag drives it, not the state column. Auto-catch filters its candidates to ongoing. `✕` writes a `blur@now` (→ Recent, still visible/resumable); archive sets `state=archived` (hidden). A migration rewrites existing rows (manual-stopped → visible; auto-stopped → archived). **Scale-free reads:** `loadGlobalEvents(since:)` adds `ts >= now − 30 d` and `ORDER BY ts`, so `idx_events_ts` turns it into an indexed range scan whose cost is independent of total table size — years-old rows are never touched on the refresh path. 30 d is a heuristic: a focus/afk/pause open longer than that is pathological (the machine slept/restarted → startup re-derives), and the only error is a wrong menu label; **billing (`segments.get`) uses the separate full-range `loadEventsInRange` and stays exact.** Chosen over an in-memory incremental accumulator (window is sub-ms at any realistic volume and far simpler — no accumulator type, startup rebuild, or out-of-order-ts handling); the takeover notification stays in `DaemonModel.refresh`.

38. **`events.source_id` removed — `activity_id` is the sole ownership link.** The column was dead weight: for an activity event it merely copied `activities.source_id` (a stale snapshot after `enrichActivity` re-points a shell's source, and never read by anything — `GlobalState.reduce`/`Attribution` key on `kind` + `activity_id`, never source); for a global event (afk/pause) it existed only to satisfy the `NOT NULL` FK, forcing the daemon to invent fake `kairos`/`idle` "sources" that own no activity and pollute the `sources` table. So `events` now has no `source_id`: an event's owner is `activity_id` alone (non-NULL = the activity's own event; NULL = a global daemon event), and afk-vs-pause is already carried by `kind` — an event-dedicated origin field would just re-encode `kind`. An activity event's source is joined from `activities` when needed (single source of truth, no denorm, no enrich-staleness). Considered and rejected: a nullable `source_id` set only on global rows (leaves a half-dead column) and a dedicated `origin` column (kind-derivable redundancy). Migration v3 drops `idx_events_source` then the column (SQLite 3.35+ `ALTER … DROP COLUMN`; DDL, so the append-only triggers don't fire) and deletes the orphan `kairos`/`idle` sources. `appendEvent` loses its `sourceId` param; the idle sampler and pause path stop resolving a source; `DaemonSources` is gone. Billing is unaffected (it never read event source).
