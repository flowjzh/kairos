...
public struct Dispatcher: Sendable {
    ...
    /// A focus transition from the `kairos` PTY wrapper. Resolve the wrapper
    /// session to its activity and append `focus`/`blur`; if the mapping isn't
    /// known yet (the report beat the first hook), buffer it for flush on
    /// registration.
    ...
    private func appendFocus(_ target: SessionRegistry.Target, focused: Bool, ts: Double, store: Store) async throws {
        ...
        _ = try await store.appendEvent(activityId: activityId, sourceId: sourceId, kind: focused ? .focus : .blur, ts: ts)
    }
