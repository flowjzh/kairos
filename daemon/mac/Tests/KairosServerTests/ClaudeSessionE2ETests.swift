...
/// End-to-end: drive a realistic Claude Code session through the dispatcher →
/// store → attribution, then read segments back — the full ingest-to-attribution
/// path minus the socket wire. The hook → RPC mapping itself now lives in the
/// Rust plugin (and is tested there); here we build the same requests inline.
@Suite
struct ClaudeSessionE2ETests {
    private let d = Dispatcher()

    /// Dispatch a hook event at time `ts` exactly as the plugin binary would:
    /// build the request inline (mirroring the Rust `kairos-claude-code` mapping),
    /// then hand it to the dispatcher with `now` pinned to that instant.
    private func hook(_ event: String, session: String, project: String, ts: Double, store: Store) async throws {
        ...
        case "SessionStart":
            ...
            request = try RequestEnvelope(method: .activitiesOpen, params: Wire.encodeValue(
                ActivitiesOpenParams(source: "claude-code", externalId: session, project: project, title: nil, metadata: metadata)))
        case "UserPromptSubmit":
            request = try RequestEnvelope(method: .eventsPost, params: Wire.encodeValue(
                EventsPostParams(activity: activity, kind: "ai_submit", ts: ts, payload: nil)))
        case "Stop":
            request = try RequestEnvelope(method: .eventsPost, params: Wire.encodeValue(
                EventsPostParams(activity: activity, kind: "ai_stop", ts: ts, payload: nil)))
        case "SessionEnd":
            request = try RequestEnvelope(method: .activitiesClose, params: Wire.encodeValue(
                ActivitiesCloseParams(source: "claude-code", externalId: session, ts: ts)))
        default:
